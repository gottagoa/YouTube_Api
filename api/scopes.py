...
# где ему будет предложено предоставить доступ к запрошенным областям вашему приложению. 
...
'''
Чтобы программно отозвать токен, ваше приложение отправляет запрос на
https://oauth2.googleapis.com/revoke и включает токен в качестве параметра:\
Примечание. Конечная точка Google OAuth 2.0 для отзыва токенов поддерживает JSONP и отправку форм. 
Он не поддерживает совместное использование ресурсов между источниками (CORS).
'''
